...
void gui_show_ncov_data(struct ncov_data china, struct ncov_data global)
{
    ...
    gui_show_chn_string((LCD_SIZE_X-16*4)/2, y0, "国内疫情", CLR_FONT);
    gui_show_line_data(y0+20, "现存确诊", china.currentConfirmedCount, "较昨日", china.currentConfirmedIncr, CLR_FONT, CLR_NUM);
    gui_show_line_data(y0+40, "累计确诊", china.confirmedCount, "较昨日", china.confirmedIncr, CLR_FONT, CLR_NUM);
    gui_show_line_data(y0+60, "累计治愈", china.curedCount, "较昨日", china.curedIncr, CLR_FONT, CLR_NUM);
    gui_show_line_data(y0+80, "现存重症", china.seriousCount, "较昨日", china.seriousIncr, CLR_FONT, CLR_NUM);
    gui_show_line_data(y0+100, "累计死亡", china.deadCount, "较昨日", china.deadIncr, CLR_FONT, CLR_NUM);

    /* 全球疫情 */
    gui_show_chn_string((LCD_SIZE_X-16*4)/2, 140, "全球疫情", CLR_FONT);
    gui_show_line_data(y0+140, "现存确诊", global.currentConfirmedCount, "较昨日", global.currentConfirmedIncr, CLR_FONT, CLR_NUM);
    gui_show_line_data(y0+160, "累计治愈", global.curedCount, "较昨日", global.curedIncr, CLR_FONT, CLR_NUM);
    gui_show_line_data(y0+180, "累计死亡", global.deadCount, "较昨日", global.deadIncr, CLR_FONT, CLR_NUM);
    
    gui_show_chn_string(100, 222, "更新于", CLR_FONT);
...
{
    gui_show_chn_string(0, y, str1, clr_str);        //累计确诊
    gui_show_F8X16_Char(64, y, 10, clr_str);       //: -> 10
    gui_show_num(72, y, num1, clr_num);        //12345678
    gui_show_chn_string(136, y, str2, clr_str);      //较昨日
